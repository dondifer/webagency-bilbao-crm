...
export default async (req) => {
  ...
  if (req.method === "GET") return json((await s.get("leads", { type: "json" })) || []);

  if (req.method === "POST") {
    const body = await req.json();
    const lista = (await s.get("leads", { type: "json" })) || [];
    const nuevo = { ...body, id: body.id || crypto.randomUUID(), created_at: new Date().toISOString() };
    lista.push(nuevo);
    await s.setJSON("leads", lista);
  ...
  if (req.method === "DELETE") {
    const lista = (await s.get("leads", { type: "json" })) || [];
    await s.setJSON("leads", lista.filter((x) => x.id !== id));
    return json({ ok: true });
  }
